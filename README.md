...
```go
// Groups and tiers modelled as one tag type (they could also be separate types).
...
Real projects accumulate parallel enums that model the same concept: one
generated by sqlboiler from the DB schema, one generated from an OpenAPI spec,
one hand-written in the business models. They should represent the same set,
so casting between them shouldn't need per-pair boilerplate.

```go
type SqlSuit struct{ enum.StringEnum[SqlSuit] }
...
the cast, mirroring how `Scan(nil)` treats a NULL column. A registered member
whose backing value doesn't name a member of the target type yields
`(zero, false)` / `*InvalidValueError[To]` / panic respectively.

If the linter isn't in the loop, assert the invariant at runtime — in an
`init()` or a test:

```go
if err := enum.SameValues[SqlSuit, ApiSuit](); err != nil {
	panic(err) // reports values present on only one side (or a kind mismatch)
}
```

`enumcheck` verifies the same invariant statically — see rule 4 below.
